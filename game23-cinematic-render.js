...
const cinePostMat=new THREE.ShaderMaterial({depthTest:false,depthWrite:false,toneMapped:true,uniforms:{tColor:{value:cineTarget.texture},tDepth:{value:cineTarget.depthTexture},uResolution:{value:new THREE.Vector2(1,1)},uTime:{value:0},uNear:{value:camera.near},uFar:{value:camera.far},uContact:{value:1}},vertexShader:'varying vec2 vUv;void main(){vUv=uv;gl_Position=vec4(position.xy,0.0,1.0);}',fragmentShader:`
  varying vec2 vUv;uniform sampler2D tColor;uniform sampler2D tDepth;uniform vec2 uResolution;uniform float uTime;uniform float uNear;uniform float uFar;uniform float uContact;
  ...
  void main(){vec2 px=1.0/uResolution;vec3 c=texture2D(tColor,vUv).rgb;
    ...
    float edge=smoothstep(.12,.45,(hi-lo)/max(hi,.15));c=mix(c,(n+s+e+w)*.25,edge*.23);
    ...
    float lum=luma(c);c=mix(vec3(lum),c,.88);c*=vec3(.99,1.015,1.025);c+=vec3(.016,.024,.029)*(1.0-smoothstep(.0,.3,lum));
...
renderer.render=function(s,c){
  ...
  cinePostMat.uniforms.uTime.value=t;cinePostMat.uniforms.uContact.value=cineQuality==='balanced'?.6:1;
