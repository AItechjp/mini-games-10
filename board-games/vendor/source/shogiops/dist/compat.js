...
export function parseFairyUsi(usi, rules) {
    // drops: +N*3b == G*3b
    // moves: 1a1b- == 1a1b+
    if (rules === 'kyotoshogi') {
        if (usi[0] === '+') {
            const dropUnpromoted = parseUsi(usi.slice(1));
            if (dropUnpromoted) {
                const promotedRole = promote('kyotoshogi')(dropUnpromoted.role);
                if (promotedRole)
                    return { role: promotedRole, to: dropUnpromoted.to };
            }
        }
        else if (usi.includes('-'))
            return parseUsi(`${usi.slice(0, -1)}+`);
    }
    // drops: C*2b == P*2b
