...
export function initialSfen(rules) {
    switch (rules) {
        case 'chushogi':
...
export function roleToForsyth(rules) {
    switch (rules) {
        case 'chushogi':
...
export function forsythToRole(rules) {
    switch (rules) {
        case 'chushogi':
...
export function pieceToForsyth(rules) {
    return (piece) => {
...
export function forsythToPiece(rules) {
    return (s) => {
...
export function parseBoardSfen(rules, boardPart) {
    ...
    for (let i = 0; i < boardPart.length; i++) {
        ...
        if (c === '/' && file < 0) {
            ...
            if (!isNaN(step)) {
                ...
                if (!piece)
                    return Result.err(new SfenError(InvalidSfen.BoardPiece));
...
export function parseHands(rules, handsPart) {
...
export function parseSfen(rules, sfen, strict) {
    if (sfen === 'startpos')
        sfen = initialSfen(rules);
    const parts = sfen.split(/[\s_]+/);
    // Board
    const boardPart = parts.shift();
    const board = parseBoardSfen(rules, boardPart);
    // Turn
    ...
    if (!defined(moveNumber))
        return Result.err(new SfenError(InvalidSfen.MoveNumber));
    if (parts.length > 0)
...
export function makeBoardSfen(rules, board) {
    const dims = dimensions(rules);
    let sfen = '';
    let empty = 0;
...
export function makeHandSfen(rules, hand) {
...
export function makeHandsSfen(rules, hands) {
...
function dobutsuRoleToForsyth(role) {
    switch (role) {
        case 'king':
            return 'k';
        case 'pawn':
...
function dobutsuForsythToRole(ch) {
    switch (ch.toLowerCase()) {
        case 'k':
        case 'l': // lion
            return 'king';
