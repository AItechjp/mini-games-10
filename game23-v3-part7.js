...
addEventListener('keydown',e=>{if(e.isComposing||e.ctrlKey||e.metaKey||e.altKey||document.documentElement.hasAttribute('data-aitech-guide-open')||e.composedPath().some(n=>n?.matches?.('input,select,textarea,button,a,dialog,[contenteditable]')))return;if(!state.running||(!pointerLocked&&!frame.contains(document.activeElement)))return;keys.add(e.code);if(e.code==='Space'){e.preventDefault();shoot();}});addEventListener('keyup',e=>keys.delete(e.code));
