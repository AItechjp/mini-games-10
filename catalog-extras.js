(() => {
  ...
  const extras = [
    ...
    { n: 17, title: 'SHARED WHITEBOARD', badge: 'REALTIME / WHITEBOARD', desc: '指・タッチペン・マウスで描ける共有ホワイトボード。部屋コードで相手へリアルタイム同期、途中参加時の盤面復元・全消去同期つき。', href: 'whiteboard.html', action: '共有ボードを開く' },
  ...
  const party = [
    [24,'REACTOR RELAY','CO-OP / 5 MIN','制御コードをつなぎ、原子炉を安定化。'],
    [25,'METEOR DEFENSE','CO-OP / DEFENSE','迫る隕石を迎撃。基地HPを守り切れ。'],
    [26,'CARGO RUSH','VS / RISK','3レーンから貨物を回収。安全策か高配当か。'],
    [27,'VIRUS PURGE','CO-OP / SPEED','感染ノードを高速駆除し、ネットワークを救う。'],
    ...
    [34,'KITCHEN CHAOS','CO-OP / COOKING','注文どおりに食材を連携投入。'],
    ...
    [38,'LASER MAZE','CO-OP / MAZE','共有ドローンを誘導し迷路を5回突破。'],
    [39,'CASTLE SIEGE','VS / STRATEGY','攻撃・防御・チャージで相手の城を破壊。'],
    [40,'FISHING FRENZY','VS / TIMING','ゲージ中央でHOOK。先に100kg釣り上げる。'],
    [41,'FIRE BRIGADE','CO-OP / DEFENSE','広がる火災を消火し、建物を守る。'],
    [42,'RHYTHM RELAY','CO-OP / RHYTHM','4ビートをつないでライブ成功を狙う。'],
    [43,'CROWN RACE','VS / RACE','安全道・近道・ワープを選び、先に王冠へ。']
  ];
  party.forEach(([n,title,badge,baseDesc]) => {
    const coop = badge.startsWith('CO-OP');
    extras.push({
      n, title,
      badge: online ? badge : `${coop ? 'CPU PARTNER' : 'VS CPU'} / 5 MIN`,
      desc: online ? `${baseDesc} 2人オンライン対応。` : `${baseDesc} ${coop ? 'CPUパートナーと協力。' : 'CPUと対戦。'}`,
      href: `party.html?game=${n}&mode=${online ? 'online' : 'solo'}`,
      action: online ? '2人プレイを開く' : '1人で遊ぶ'
    });
  });
