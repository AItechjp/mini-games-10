...
await build({configFile:false,root:join(source,'aitech'),base:'/commons/',plugins:[react()],resolve:{alias:{'@':source}},css:{postcss:source},publicDir:false,build:{outDir:output,emptyOutDir:false,assetsInlineLimit:100000,rollupOptions:{input:{index:input,camera:join(source,'aitech/camera.tsx')},output:{entryFileNames:chunk=>chunk.name==='camera'?'camera-auth.js':'assets/[name]-[hash].js'}}}});
...
await cp(join(source,'aitech/auth.css'),join(output,'auth.css'));
...
camera=camera.replace('<body>','<body><div id="camera-auth"></div><div id="camera-site" hidden>').replace('</body>','</div><script type="module" src="/commons/camera-auth.js"></script></body>').replace('</head>','<link rel="stylesheet" href="/commons/auth.css"><style>[hidden]{display:none!important}.camera-auth-ready{display:contents}</style></head>');
