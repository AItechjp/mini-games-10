...
export default function OpeningDirectory({kind,initial,serverNow}:{kind:OpeningKind;initial:OpeningSnapshot;serverNow:string}){
  ...
  const uncertainty=snapshot.records.filter(r=>r.kind===kind&&r.status==='uncertain'&&r.openingDate>=range.start&&r.openingDate<=range.end&&regionMatches(r,region)&&matchesSearch([r.name,r.prefecture,r.address,r.genre].join(' '),query)&&(!selectedDay||r.openingDate.startsWith(selectedDay)));
  ...
  return <div className={'opening-app opening-'+kind}>
    ...
    <header className="opening-header"><a href="/commons/" className="opening-back"><ArrowLeft size={18}/>COMMONS</a><span>NEW OPENINGS</span><a href={'/commons/openings/'+counterpart+'?region='+encodeURIComponent(region)}>{kind==='ramen'?'サウナ版へ':'ラーメン屋版へ'}<ArrowUpRight size={16}/></a></header>
    <main className="opening-main">
      ...
      <nav aria-label="開店情報の種類" className="opening-kind-nav"><a href={'/commons/openings/ramen/?region='+encodeURIComponent(region)} aria-current={kind==='ramen'?'page':undefined}><Soup size={18}/>ラーメン屋</a><a href={'/commons/openings/sauna/?region='+encodeURIComponent(region)} aria-current={kind==='sauna'?'page':undefined}><Flame size={18}/>サウナ</a></nav>
