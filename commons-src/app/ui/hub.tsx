...
export default function Hub() {
  ...
  const search=query.normalize('NFKC').trim().toLocaleLowerCase('ja');
  const filtered=useMemo(()=>sites.filter(site=>(tab!=='favorites'||favorites.includes(site.id))&&[String(sites.indexOf(site)+1),siteGroups.find(g=>g.sites.includes(site))?.name,site.name,site.description,site.label,site.keywords].join(' ').normalize('NFKC').toLocaleLowerCase('ja').includes(search)),[search,tab,favorites]);
  ...
  return <SidebarProvider style={{'--sidebar-width':'252px'} as React.CSSProperties}>
    ...
    <div className="grow">
      <header className="topbar">
        ...
        <div className="commons-top-tools"><a className="commons-game-link" href="/games.html">ゲーム集</a><div className="search-box"><Search/><input aria-label={sites.length+'サイトを検索'} placeholder="サイトを検索" value={query} onChange={e=>setQuery(e.target.value)}/></div></div>
      </header>
