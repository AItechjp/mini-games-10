...
function renderProviders(items){
  ...
  $('#result-count').textContent=items.length+'サイト';
  $('#result-count').classList.remove('edited');
}
...
function hotelValues(){return {prefecture:$('#prefecture').value,destination:$('#destination').value.trim(),checkin:$('#checkin').value,checkout:$('#checkout').value,adults:Number($('#adults').value),rooms:Number($('#rooms').value),sort:$('#sort').value};}
...
function submit(move=false){
  ...
  renderProviders(hotel?hotelSearches(values):rentalSearches(values,stations));
  if(hotel){
...
if(hotel){
  ...
  setValue('destination',params.get('destination')?.slice(0,80)||'');
  ...
  $('#reset').addEventListener('click',()=>{setValue('prefecture','愛知県');setValue('destination','');setValue('checkin',todayJST());setValue('checkout',addDays(todayJST(),1));$('#checkout').min=addDays(todayJST(),1);setValue('adults','2');setValue('rooms','1');setValue('sort','price');submit(false)});
  ...
  $('#prefecture').addEventListener('change',()=>{setValue('station-query','');fillStations();$('#summary').hidden=true;$('#providers').innerHTML='<div class="empty"><h3>駅を選んでください</h3><p>選んだ県の駅に切り替えました。駅・徒歩・間取りを指定して検索できます。</p></div>';$('#result-count').textContent='駅の選択待ち'});
