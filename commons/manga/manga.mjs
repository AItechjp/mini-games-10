...
await load();setInterval(()=>{if(document.hidden)return;const left=Math.max(0,Math.ceil((nextRefresh-Date.now())/1000));$('next-refresh').textContent=busy?'新着を確認中':'次の画面更新まで '+left+'秒';if(!left)load();},1000);document.addEventListener('visibilitychange',()=>{if(!document.hidden&&Date.now()>=nextRefresh)load();});
