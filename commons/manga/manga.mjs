...
try{const r=await fetch('./sources.json',{cache:'no-cache'});if(!r.ok)throw new Error();configured=await r.json();syncFilters();renderSources();renderQuick();renderMeta();}catch{$('error').textContent='配信元の一覧を読み込めませんでした。ページを再読み込みしてください。';$('error').hidden=false;}
