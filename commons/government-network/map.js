...
function normalizeData(raw){
 ...
 $('kind').innerHTML=opts([['overview','府省の全体像'],['all','行政機関・独立行政法人すべて'],['agency','行政機関'],['iaa','独立行政法人']]);
 }
...
function update(){
 ...
 else{const allowed=m?descendants(m):null;filtered=nodes.filter(n=>(!allowed||allowed.has(n.id))&&(!q||norm([n.name,n.aliases?.join(' '),n.abbreviation,n.note].join(' ')).includes(q))&&((q&&k==='overview')||k==='all'||k==='iaa'&&n.type==='iaa'||k==='agency'&&n.type!=='iaa'||k==='overview'&&(['cabinet','ministry','secretariat'].includes(n.type)||n.name==='内閣官房')));}
...
function renderGraph(){
 ...
 bounds={w:Math.max(1000,...graphNodes.map(n=>n.x+n.w+30)),h:Math.max(500,...graphNodes.map(n=>n.y+n.h+80))};
 const byId=new Map(graphNodes.map(n=>[n.id,n]));
 const paths=graphEdges.map(e=>{const a=byId.get(e.source),b=byId.get(e.target);if(!a||!b)return '';const x=a.x+a.w,y=a.y+a.h/2,endX=b.x,endY=b.y+b.h/2,mx=(x+endX)/2;return `<path class="edge ${esc(e.type)}" d="M${x},${y} C${mx},${y} ${mx},${endY} ${endX-7},${endY}" marker-end="url(#arrow)"><title>${esc(a.name)} → ${esc(b.name)}（${e.type==='supervision'?'所管':e.type==='reemployment'?'再就職の公表記録':'所属'}）</title></path>`}).join('');
...
function setView(v){view=v;$('graph-view').setAttribute('aria-pressed',v==='graph');$('list-view').setAttribute('aria-pressed',v==='list');document.querySelector('.graph-wrap').style.display=v==='graph'?'block':'none';$('detail').style.display=v==='graph'?'block':'none';$('table-view').style.display=v==='list'?'block':'none';$('pagination').hidden=v==='list'||filtered.length<=PAGE_SIZE;if(v==='graph')fit()}
...
function fit(){const box=$('graph').getBoundingClientRect();if(!box.width||!box.height)return;const k=Math.min(1.1,(box.width-38)/bounds.w,(box.height-100)/bounds.h);transform={k,x:(box.width-bounds.w*k)/2,y:50+(box.height-100-bounds.h*k)/2};applyTransform()}
...
$('page-size').onchange=()=>{PAGE_SIZE=Number($('page-size').value);page=0;renderGraph()};$('fit').onclick=fit;$('zoom-in').onclick=()=>zoom(1.25);$('zoom-out').onclick=()=>zoom(.8);
