...
export function fallbackPortrait(c,size=120){const hash=h(c.en),[light,mid,dark]=palettes[c.tribe]||palettes.warrior,id=`a${hash}${size}`,rarity=c.level===4?'#edd292':mid;let figure='';const eyes='<path d="M43 37l5 2-4 1M57 37l-5 2 4 1" stroke="#f7f2cb" stroke-width="1.8"/>';
 ...
 else if(c.tribe==='beast')figure=`<path d="M29 62Q22 54 12 60L18 74 32 73M70 64Q84 49 84 38Q99 64 77 79" fill="${mid}"/><path d="M31 47L23 24 43 31 59 31 81 21 72 48 77 68 66 79 69 90 52 87 47 80 39 90 22 88 29 75 23 62Z" fill="url(#${id})"/><path d="M32 38L30 30 43 39M64 39L73 29 68 43" fill="${dark}"/><path d="M33 48L48 53 64 47 60 65 49 74 37 66Z" fill="${light}"/><path d="M38 47l8 2M56 48l7-3" stroke="#ffee9d" stroke-width="3"/><path d="M44 57l10-1-5 7Z" fill="${dark}"/><path d="M37 60l4 10 5-5m6 0 5 5 5-11" fill="#fff0cc"/><path d="M27 88l4-6m5 6 2-7m19 5 1-5m7 7-1-8" stroke="${light}" stroke-width="2"/>`;
 else if(c.tribe==='undead')figure=`<path d="M39 51L25 73 13 88 37 83 48 93 59 82 88 89 71 69 61 50" fill="url(#${id})"/><path d="M37 30Q39 15 51 16Q69 17 66 34L61 49 40 47Z" fill="${light}"/><path d="M41 32L48 34 46 40 39 38M54 35l8-4-1 8-7 1M49 40l-2 5h5" fill="${dark}"/><path d="M43 44v7m6-6v8m6-8v6" stroke="${dark}" stroke-width="2"/><path d="M50 54v26m-8-21 8 3 10-3m-18 7 8 3 8-3m-16 7 8 3 8-3" fill="none" stroke="${light}" stroke-width="3"/><path d="M34 55L23 67 16 58M66 54L77 67 88 55" stroke="${mid}" stroke-width="5" fill="none"/><path d="M15 60L7 56m10 4-2-9m72 6 9-5m-10 4 2-11" stroke="${light}" stroke-width="2"/>`;
 else figure=`<path d="M19 45L48 31 83 46 82 72 49 89 20 72Z" fill="${dark}" stroke="${mid}" stroke-width="2"/><path d="M20 45L49 61 83 46M49 62v26" stroke="${light}" stroke-width="2"/><path d="M23 38L49 25 80 39 49 54Z" fill="url(#${id})" stroke="${light}"/><path d="M37 32l22 14m-14 4 23-13" stroke="${dark}" stroke-width="3"/><circle cx="50" cy="19" r="6" fill="${light}"/><path d="M50 1v8M35 7l5 6M64 8l-5 6" stroke="${light}" stroke-width="2"/>`;
