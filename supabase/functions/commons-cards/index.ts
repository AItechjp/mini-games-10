import { DM_ORIGIN, PAGE_SIZE, parseSearch, parseSets, parseDetail, searchParams, validId } from './core.mjs';
...
async function upstream(path: string, body?: URLSearchParams) {
  ...
  const r = await fetch(DM_ORIGIN + path, { method: body ? 'POST' : 'GET', body,
    redirect: 'error', signal: AbortSignal.timeout(16000),
    headers: { 'User-Agent': 'AItech-Card-Library/1.0 (+https://aitechd.com/commons/duel-masters-flavor/)', 'Accept': 'text/html' } });
...
export async function handler(request: Request) {
  ...
  try {
    if (q.get('kind') === 'detail') {
      ...
      return send(await cached('detail:' + id, 86400000, async () => parseDetail(await upstream('/card/detail/?id=' + encodeURIComponent(id)), id)));
    }
    if (q.get('kind') === 'sets') return send(await cached('sets', 3600000, async () => ({ sets: parseSets(await upstream('/card/')), fetchedAt: new Date().toISOString() })));
    if (q.get('kind') !== 'search') return send({ error: '検索種別が不正です。' }, 400);
    let spec; try { spec = searchParams(q); } catch (e) { return send({ error: (e as Error).message }, 400); }
    const result = await cached('search:' + spec.params.toString(), 900000, async () => ({ ...parseSearch(await upstream('/card/', spec.params)), fetchedAt: new Date().toISOString() }));
    const cards = result.cards.slice(spec.offset, spec.offset + PAGE_SIZE);
    const originalQuery: Record<string, string | string[]> = {};
    for (const [key, value] of spec.params) {
      if (key.endsWith('[]')) { const name = key.slice(0, -2); (originalQuery[name] ||= [] as string[]); (originalQuery[name] as string[]).push(value); }
      else originalQuery[key] = value;
    }
    return send({ cards, total: result.total, page: spec.page, pageSize: PAGE_SIZE, fetchedAt: result.fetchedAt,
      sourceUrl: DM_ORIGIN + '/card/?v=' + encodeURIComponent(JSON.stringify(originalQuery)),
      coverage: 'official-catalog', releaseScope: 'official-listed' });
