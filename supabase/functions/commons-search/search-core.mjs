...
export function validateHotel(s, today = todayJST()) {
  ...
  if([s.area,s.subarea].some(x=>x&&!/^[A-Za-z0-9_-]{1,60}$/.test(x))) return '地域を選び直してください。';
...
export function hotelSearches(s) {
  ...
  return [
    ...
    {id:'rakuten',name:'楽天トラベル',mark:'R',type:'日程を引き継ぐ',note:s.destination.trim()?'都道府県・日程・人数・料金順を渡します。市区町村や駅、ホテル名は楽天トラベルで再指定してください。':'指定した都道府県と日程で検索。ポイント・クーポン適用条件は予約画面で確認できます。',capabilities:['宿泊日','大人・部屋数',s.sort==='price'?'料金の安い順':'おすすめ順'],url:link('https://search.travel.rakuten.co.jp/ds/vacant/searchVacant/',{f_dai:'japan',f_chu:rakutenPrefectures[s.prefecture],f_shou:s.area||'',f_sai:s.subarea||'',f_cd:s.subarea?'04':s.area?'03':'02',f_nen1:iy,f_tuki1:im,f_hi1:id,f_nen2:oy,f_tuki2:om,f_hi2:od,f_otona_su:s.adults,f_heya_su:s.rooms,f_sort:s.sort==='price'?'hotel_kin_low':'hotel',f_sort_cate:'hotel',f_tab:'hotel',f_hyoji:30})},
