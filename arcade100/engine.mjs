...
export function action(s,side,raw){if(!Number.isInteger(side)||side<0||side>=s.players.length||s.phase!=='playing')return false;const a=cleanAction(raw);if(!a||!['cell','primary','secondary','ability','choice','select','card','confirm','undo','pass','buy','send','bid','hint','chat','answer','draw','meeting','vote'].includes(a.type))return false;const valid=ENGINES[s.family].act(s,side,a);if(valid)s.version++;return !!valid;}
