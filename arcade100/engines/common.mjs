...
export function endClock(s){if(s.phase!=='playing'||s.t<s.duration)return;if(s.mode==='coop')finish(s,s.progress>=s.target?'team':'failed',s.progress>=s.target?'共同ミッション達成':'時間切れ');else if(s.n===20)finish(s,s.players[0].hp>0?0:'hunters','討伐の制限時間終了');else if(s.data.teamGame||s.data.teams){const scores=s.data.teams||[0,1].map(team=>s.players.filter(p=>p.team===team).reduce((sum,p)=>sum+p.score,0));finish(s,scores[0]===scores[1]?'draw':'team'+(scores[0]>scores[1]?0:1),'チーム戦終了');}else byScore(s);}
