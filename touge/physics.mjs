// Deterministic, frame-rate-independent arcade vehicle dynamics. Metres and seconds.
...
export function cornerSpeed(k){return clamp(Math.sqrt(10.2/(Math.abs(k)+.0011)),17,53);}
export function step(p,dt,i,track,assist=true,difficulty='normal'){
 if(p.finished)return;p.t+=dt;p.cooldown=Math.max(0,p.cooldown-dt);
 const a=track.at(p.s),look=track.at(p.s+Math.max(18,p.v*1.5));
 let steer=clamp(i.steer||0,-1,1),gas=clamp(i.gas||0,0,1),brake=clamp(i.brake||0,0,1);
 ...
 let autoBrake=assist?clamp((p.v-safe-2)/10,0,.78):0;
 const off=Math.abs(p.x)>4.9;const targetSlip=p.steer*(drifting?.43:.045)*(p.v/38);
 p.slip=damp(p.slip,clamp(targetSlip,-.57,.57),drifting?4:7,dt);
 const acceleration=gas*(8.5/(1+p.v*.028))-(Math.max(brake,autoBrake)*14)-.18-p.v*p.v*.00125-(off?7:0)-(drifting?1.7:0);
 ...
 if(p.drift){let points=(p.v*1.4+Math.abs(p.slip)*100)*dt;p.score+=points;p.combo+=points;p.bestCombo=Math.max(p.bestCombo,p.combo);}else p.combo=Math.max(0,p.combo-45*dt);
 const old=p.s;p.s=Math.min(track.length,p.s+p.v*dt*Math.cos(p.slip*.4));
 ...
 if(p.s>=track.length){p.t-=dt*(p.s===old?0:clamp((old+p.v*dt-track.length)/Math.max(.01,p.v*dt),0,1));p.finished=true;p.reason='finish';}
