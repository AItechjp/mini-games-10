...
import {BUILDINGS,ROADS,POI,CACHES,clamp,angle,rng,target,distance} from './core.mjs';
...
function hull(sections){const pos=[],ind=[],normals=[];for(const [z,w,low,high] of sections){pos.push(-w,low,z,w,low,z,w,high,z,-w,high,z);}for(let j=0;j<sections.length-1;j++)for(let k=0;k<4;k++){let a=j*4+k,b=j*4+(k+1)%4,c=b+4,d=a+4;ind.push(a,b,d,b,c,d);}ind.push(0,3,1,1,3,2);let a=(sections.length-1)*4;ind.push(a,a+1,a+3,a+1,a+2,a+3);const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(pos,3));g.setIndex(ind);g.computeVertexNormals();return g;}
function carModel(type=0,paint=0){
 const group=new T.Group(),metal=M(PAINT[paint%PAINT.length],.24,.75,{envMapIntensity:1.4}),glass=M(0x19333f,.13,.7),trim=M(0x111519,.46,.4),silver=M(0x9aabad,.2,.9),rubber=M(0x111214,.85),red=M(0x77180f,.25,.4,{emissive:0xff230e,emissiveIntensity:.75}),lamp=M(0xd6e6e6,.17,.4,{emissive:0xffe6bd,emissiveIntensity:3.5});
 const buckets=new Map();const add=(geo,mat,x=0,y=0,z=0,rx=0,ry=0,rz=0)=>{geo.applyMatrix4(new T.Matrix4().compose(V(x,y,z),new T.Quaternion().setFromEuler(new T.Euler(rx,ry,rz)),V(1,1,1)));if(!buckets.has(mat))buckets.set(mat,[]);buckets.get(mat).push(geo);};
 const box=(w,h,d,mat,x,y,z,ry=0)=>add(new T.BoxGeometry(w,h,d),mat,x,y,z,0,ry);
 const tall=type===2?.32:type===3?.55:0;
 add(hull([[-2.25,.78,.46,.66],[-1.85,.97,.38,.92],[1.65,.94,.42,.85],[2.2,.73,.5,.72]]),metal);
 add(hull([[-1.28,.81,.88,1.03+tall],[-.72,.72,.9,1.51+tall],[.54,.70,.9,1.50+tall],[1.15,.82,.86,.94]]),glass);
 box(1.45,.075,1.2,metal,0,1.53+tall,-.08);
 for(const x of [-.94,.94]){box(.065,.13,2.7,metal,x,.82,0);box(.09,.08,4.0,trim,x,.48,0);box(.04,.025,.22,silver,x*1.02,1.02,-.42);box(.12,.07,.28,metal,x*1.13,1.05,.56);box(.04,.55+tall,.07,metal,x*.78,1.16+tall/2,-.1);box(.04,.025,1.5,silver,x*.95,.95,-.25);}
 box(1.58,.12,.16,trim,0,.48,2.2);box(1.48,.11,.1,trim,0,.48,-2.2);box(.72,.14,.04,trim,0,.65,2.205);
 for(const x of [-.56,.56]){box(.38,.065,.1,lamp,x,.76,2.12);box(.32,.095,.08,red,x,.8,-2.15);box(.13,.09,.16,silver,x,.41,-2.22);}
 box(.38,.12,.015,M(0xdbe0d3,.6),0,.62,-2.265);
 for(const x of [-.96,.96])for(const z of [-1.4,1.37]){
  add(new T.CylinderGeometry(.43,.43,.24,18),rubber,x,.44,z,0,0,Math.PI/2);
  add(new T.CylinderGeometry(.28,.28,.255,14),silver,x,.44,z,0,0,Math.PI/2);
  add(new T.CylinderGeometry(.17,.17,.263,10),trim,x,.44,z,0,0,Math.PI/2);
  for(let k=0;k<5;k++){const a=k*Math.PI*2/5;box(.27,.038,.36,silver,x,.44+Math.sin(a)*.11,z+Math.cos(a)*.11,a);}
 }
 if(type===0){box(1.4,.045,.26,metal,0,1.01,-1.9);box(.06,.22,.06,trim,-.55,.92,-1.9);box(.06,.22,.06,trim,.55,.92,-1.9);}
 if(type===1){const taxi=M(0xefbc45,.3,.3);box(.62,.15,.3,taxi,0,1.68,0);}
 if(type===2){for(const x of [-.57,.57])box(.05,.04,1.7,trim,x,1.96,-.16);}
 if(type===3)box(1.58,.9,1.32,metal,0,1.35,-.72);
 let sirens=[];
 if(type===4){box(1.14,.08,.3,trim,0,1.63,0);for(const [x,c] of [[-.35,0x166fff],[.35,0xff1e13]]){const mat=M(c,.2,0,{emissive:c,emissiveIntensity:2.5}),m=new T.Mesh(new T.BoxGeometry(.42,.11,.22),mat);m.position.set(x,1.7,0);group.add(m);sirens.push(m);}box(1.8,.025,.54,M(0xd4dcda,.4,.2),0,.95,-1.6);}
 for(const [mat,geos] of buckets){const g=mergeGeometries(geos,false),mesh=new T.Mesh(g,mat);mesh.castShadow=true;mesh.receiveShadow=true;group.add(mesh);for(const g of geos)g.dispose();}
 group.userData.sirens=sirens;group.userData.brake=red;return group;
}
...
export class CityView{
 ...
 buildWorld(){
  ...
  B(-80,.18,80,115,.05,115,grass);B(-80,.2,80,6,.05,115,concrete);B(-80,.21,80,115,.05,6,concrete);const pool=new T.Mesh(new T.CylinderGeometry(16,16,.25,64),M(0x315363,.18,.65));pool.position.set(-80,.37,80);pool.receiveShadow=true;sc.add(pool);this.batch(cyl,stone,-80,.4,80,33,.3,33);sc.add(pool);B(-80,3.5,80,2,6.5,2,stone,.6);
 ...
 setWeather(rain){this.rain.visible=!!rain;this.scene.fog.density=rain?.00175:.00095;}
 setQuality(q){this.quality=['low','medium','high'].includes(q)?q:'medium';const low=this.quality==='low';this.renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,low?.85:this.quality==='high'?1.5:1.1));this.renderer.shadowMap.enabled=!low;this.sun.shadow.mapSize.set(this.quality==='high'?2048:1024,this.quality==='high'?2048:1024);if(this.sun.shadow.map){this.sun.shadow.map.dispose();this.sun.shadow.map=null;}this.resize();}
 resize(){const w=this.canvas.clientWidth||innerWidth,h=this.canvas.clientHeight||innerHeight;this.camera.aspect=w/h;this.camera.updateProjectionMatrix();this.renderer.setSize(w,h,false);if(this.composer)this.composer.setSize(w,h);}
 ...
 draw(s,localId,dt,cam={yaw:Math.PI,pitch:.2},menu=false){
  dt=Math.min(dt,.05);this.clock+=dt;const p=s.players[localId]||s.players[0],focus=V(p.x,1.25,p.z),t=s.time;const visibleDistance=this.quality==='low'?220:420;
  for(const c of s.cars){let m=this.dynamicCars.get(c.id);if(!m){m=carModel(c.type,c.paint);this.scene.add(m);this.dynamicCars.set(c.id,m);m.position.set(c.x,0,c.z);}m.visible=menu||distance(c,p)<visibleDistance;if(m.visible){m.position.lerp(V(c.x,.05,c.z),1-Math.exp(-dt*28));m.rotation.y+=angle(c.yaw-m.rotation.y)*(1-Math.exp(-dt*22));m.rotation.z=Math.sin(t*16+c.id)*Math.min(.006,Math.abs(c.speed)*.0003);m.userData.brake.emissiveIntensity=c.speed<3?1.6:.55;for(let i=0;i<m.userData.sirens.length;i++)m.userData.sirens[i].material.emissiveIntensity=s.wanted>.5&&Math.sin(t*12+i*Math.PI)>.1?6:.2;}}
  const active=new Set();for(const [kind,list]of [['p',s.players],['e',s.enemies]])for(const a of list){const id=kind+a.id;active.add(id);let m=this.actors.get(id);if(!m){m=personModel(kind==='e'?2:a.id);m.position.set(a.x,.15,a.z);this.scene.add(m);this.actors.set(id,m);}m.visible=a.hp>0&&(a.car===undefined||a.car<0)&&(menu||distance(a,p)<190);if(!m.visible)continue;m.position.lerp(V(a.x,.16,a.z),1-Math.exp(-dt*24));m.rotation.y+=angle(a.yaw-m.rotation.y)*(1-Math.exp(-dt*20));const walk=Math.min(1,(a.moving||0)/3),phase=t*(a.moving>5?12:8)+a.x*.01;for(let i=0;i<2;i++){m.userData.legs[i].rotation.x=Math.sin(phase+i*Math.PI)*.62*walk;m.userData.arms[i].rotation.x=-Math.sin(phase+i*Math.PI)*.42*walk;}if(a.shot>0){m.userData.arms[1].rotation.x=-1.45;m.userData.weapon.position.set(.28,1.28,.38);}else m.userData.weapon.position.set(.28,.86,.2);}
  ...
  const phase=s.complete?0:.12+.65*(.5-.5*Math.cos(s.elapsed/1600*Math.PI));this.sky.material.uniforms.phase.value=phase;this.sun.intensity=2.2-phase*1.55;this.hemi.intensity=1.9-phase*.45;this.sun.position.set(p.x-90,145,p.z-75);this.sun.target.position.set(p.x,0,p.z);this.sky.position.copy(this.camera.position);this.ocean.material.uniforms.time.value=t;
  ...
  else{const base=car?10.5:6.4,h=car?2.4:1.2,d=base*Math.cos(cam.pitch),height=h+base*Math.sin(cam.pitch);let desired=V(p.x-Math.sin(cam.yaw)*d,p.car>=0?height+1.1:height+1.45,p.z-Math.cos(cam.yaw)*d);const dir=desired.clone().sub(focus),len=dir.length(),ray=new T.Ray(focus,dir.normalize());let limit=len;const hit=new T.Vector3();for(let i=0;i<BUILDINGS.length;i++){if(distance(p,BUILDINGS[i])>65)continue;if(ray.intersectBox(this.colliders[i],hit))limit=Math.min(limit,focus.distanceTo(hit)-.4);}if(limit<len)desired=focus.clone().addScaledVector(dir,Math.max(1,limit));this.shake*=Math.exp(-dt*9);desired.x+=(Math.random()-.5)*this.shake;desired.y+=(Math.random()-.5)*this.shake;if(!this.cameraReady){this.camera.position.copy(desired);this.cameraReady=true;}else this.camera.position.lerp(desired,1-Math.exp(-dt*(car?8:13)));this.camera.lookAt(focus.x,focus.y+.2,focus.z);this.camera.fov=59+Math.min(10,Math.abs(car?.speed||0)*.2);this.camera.updateProjectionMatrix();}
