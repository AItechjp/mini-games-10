...
export const WALK_SPEED=4.6, SPRINT_SPEED=14;
...
function movePerson(p,dx,dz){const steps=Math.max(1,Math.ceil(Math.hypot(dx,dz)/.22));for(let k=0;k<steps;k++){if(!blocked(p.x+dx/steps,p.z,.5))p.x+=dx/steps;if(!blocked(p.x,p.z+dz/steps,.5))p.z+=dz/steps;}}
...
export function tick(s,rawInputs,delta){
 ...
 for(const p of s.players){const i=inputs[p.id];p.shot=Math.max(0,p.shot-dt);if(p.reload>0){p.reload=Math.max(0,p.reload-dt);if(p.reload===0)p.ammo=30;}
  ...
  perform(s,p,i);if(p.car<0){const speed=i.run?SPRINT_SPEED:WALK_SPEED,dx=(Math.sin(i.yaw)*i.y-Math.cos(i.yaw)*i.x)*speed*dt,dz=(Math.cos(i.yaw)*i.y+Math.sin(i.yaw)*i.x)*speed*dt;const oldX=p.x,oldZ=p.z;movePerson(p,dx,dz);p.moving=Math.hypot(p.x-oldX,p.z-oldZ)/dt;if(p.moving>.1)p.yaw=Math.atan2(dx,dz);if(i.fire)p.yaw=i.yaw;}else p.moving=Math.abs(s.cars[p.car]?.speed||0);
