...
function closeGuide(){persist();dialog.close();delete host.dataset.open;delete root.dataset.aitechGuideOpen;document.dispatchEvent(new CustomEvent('aitech:guide-close',{detail:{appId:app.id}}));if(lastFocus?.isConnected)lastFocus.focus({preventScroll:true});else triggerHost?.shadowRoot?.querySelector('button')?.focus({preventScroll:true});}
function refresh(){
...
const observer=new MutationObserver(()=>{if(!refreshTimer)refreshTimer=setTimeout(()=>{refreshTimer=undefined;refresh();},400);});observer.observe(document.body,{childList:true,subtree:true});
...
document.addEventListener('keydown',event=>{if(event.altKey&&event.shiftKey&&event.code==='KeyM'&&!event.repeat){event.preventDefault();event.stopImmediatePropagation();if(dialog?.open)closeGuide();else openGuide();}},true);
refresh();
