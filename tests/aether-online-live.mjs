...
const deck=prefix=>[...counts.flatMap((n,i)=>Array(n).fill(prefix+(i+1))),...['n1','n2','n3','n4','n5'].flatMap(id=>[id,id])].map((id,i)=>i<15?`dx-${prefix==='f'?0:1}-${i%4}-${i}`:id);
let room=null;
...
try{
 const created=await api(0,'create',{cl:'forest',deck:deck('f')});assert.equal(created.status,200,JSON.stringify(created.data));room=created.data.room;assert.match(room,/^\d{6}$/);console.log('PASS live room creation with new Dawn/Eclipse expansion card IDs');
