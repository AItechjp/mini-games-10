...
try{
 ...
 const before=host.seen.get(guest.me.id);await guest.channel.send({type:'broadcast',event:'packet',payload:{from:guest.me.id,to:host.me.id,seq:before+1000000,iv:'AAAAAAAAAAAAAAAA',data:'fake'}});const probe=waitFor(host,'message',e=>e.body.type==='probe');await guest.send(host.me.id,{type:'probe',ok:true});await probe;assert(host.seen.get(guest.me.id)<before+1000000,'tampered packet cannot advance the replay window');
