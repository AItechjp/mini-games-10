(()=>{
  ...
  function resizeCanvas(){
    ...
    viewWidth=document.documentElement.classList.contains('aitech-play-mode')?540*rect.width/Math.max(1,rect.height):960;
  ...
  function draw(){
    ctx.setTransform(canvas.width/viewWidth,0,0,canvas.height/540,0,0);
    ctx.imageSmoothingEnabled=false;
    const cam=Math.max(0,Math.min(goal-viewWidth+280,game.x-Math.min(360,viewWidth*.28))),floor=420;
    rect(0,0,viewWidth,540,'#284d58');
    const span=Math.max(1200,Math.ceil(viewWidth/170)*170+340);
    for(let i=0;i<Math.ceil(span/170)+2;i++){
      const x=((i*170-cam*.18)%span+span)%span-180,h=130+(i%4)*38;
    ...
    for(const [x,z,w] of platforms){const sx=x-cam,sy=floor-z;if(sx+w<0||sx>viewWidth)continue;rect(sx,sy,w,220,'#203e43');rect(sx,sy,w,10,'#b0dfae');rect(sx,sy+10,w,8,'#649b80');for(let j=0;j<w/60;j++)rect(sx+j*60+20,sy+43+(j%3)*24,17,6,'#34595a');}
    coins.forEach(([x,z],i)=>{if(game.taken.has(i)||x<cam-30||x>cam+viewWidth+30)return;const y=floor-z+(reduced?0:Math.sin(frameTime*3+i)*3);rect(x-cam-7,y-12,14,24,'#bc853c');rect(x-cam-10,y-8,20,16,'#bc853c');rect(x-cam-6,y-10,12,20,'#ffd685');rect(x-cam-3,y-7,3,13,'#fff0bd');});
    function flag(x,z,c,label){x-=cam;if(x<-100||x>viewWidth+100)return;rect(x,floor-z-115,5,115,'#d6e6ca');rect(x+5,floor-z-114,54,28,c);ctx.fillStyle='#e4efe5';ctx.font='11px system-ui';ctx.fillText(label,x-10,floor-z+23);}
    flag(3120,0,'#b0dfae','CAMP');flag(goal,0,'#f8b16f','GOAL');
    cat(game.x-cam,floor-game.z);
    const progress=Math.min(1,game.x/goal);rect(0,537,viewWidth,3,'#193438');rect(0,537,viewWidth*progress,3,'#b0dfae');
  }
