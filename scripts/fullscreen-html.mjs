...
export function addFullscreen(html, file, root) {
  ...
  const games = {
    ...
    'classic.html': 'classic', 'law-quiz/index.html': 'quiz-raid',
    'it-quiz/index.html': 'quiz-raid',
  ...
  if (games[name] && !html.includes('data-aitech-game-viewport')) {
    ...
    html = html.replace(/<html\b/i, `<html data-aitech-game="${games[name]}"`)
      ...
      .replace(/<\/head>/i, `<link rel="stylesheet" href="${asset}.css?v=20260914-landscape2"><script src="${asset}.js?v=20260914-quiz-split1" defer data-aitech-game-viewport></script>\n</head>`);
  }
